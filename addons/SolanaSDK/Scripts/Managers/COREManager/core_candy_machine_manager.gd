extends Node
class_name CoreCandyMachineManager
...
func try_mint(group_name:String, meta_accounts:Array) -> void:
...
#	made for default, can be overriden to a group
	var mint_group = null if group_id < 0 else group_name

	#var plugins:MintAssetArgs = mint_plugins
	#if custom_mint_plugins!=null:
		#plugins = custom_mint_plugins
	...
	var tx_data:TransactionData = await SolanaService.candy_machine_manager.mint_core_nft_with_guards(
		...
		meta_accounts,
		mint_group,
		get_guard_set_from_name(group_name),
...
func get_guard_set_from_name(group_name:String) -> GuardSet:
	if group_name == "default":
		return guard_settings.default
		
	var group_id:int = get_group_id_from_name(group_name)
	var guard_group:Group = guard_settings.groups[group_id]
	return guard_group.guards
