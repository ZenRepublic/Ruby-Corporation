...
func _process(delta):
	# Check if active input field lost focus
	if active_input_field != null and visible:
		var current_focus = get_viewport().gui_get_focus_owner()
		if current_focus != active_input_field:
...
func _update_auto_display_on_input(event):
	...
	# Only show keyboard on mobile devices
	if mobile_only and not is_mobile_device():
		return
		...
		if focus_object != null:
			...
			if click_on_input:
				if is_keyboard_focus_object(focus_object):
					active_input_field = focus_object
					_show_keyboard()
	...
	for layout in data.get("layouts"):
		...
		for row in layout.get("rows"):
			...
			for key in row.get("keys"):
				...
				# Create custom theme to override project theme
				...
				# Apply font colors
				custom_theme.set_color("font_color", "Button", font_color_normal)
				custom_theme.set_color("font_hover_color", "Button", font_color_hover)
				custom_theme.set_color("font_pressed_color", "Button", font_color_pressed)
				custom_theme.set_color("font_disabled_color", "Button", font_color_normal)
				...
				if key.has("type"):
					if key.get("type") == "switch-layout":
						...
						if style_special_keys != null:
							custom_theme.set_stylebox("normal", "Button", style_special_keys)
				...
				# Apply the custom theme to the button (after all modifications)
				new_key.theme = custom_theme
