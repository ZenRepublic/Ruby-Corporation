...
#	CHECK IF ORACLE IS ON CURVE. if on curve, means a pubkey (server oracle). if off-curve its a PDA from custom program
	print("ON CURVE?: ",oracle.is_on_curve())
	if !oracle.is_on_curve():
		game_start()
		return
